...
ROOT_FIELDS = (
    set(ROOT_STRING_FIELDS)
    | set(ROOT_ARRAY_FIELDS)
    | set(ENTRY_TYPES)
    | {
        "setting",
        "deprecated",
...
ENTRY_FIELDS = {
    ...
    | {"setting", "width", "height", "placement", "position", "open_near_click"},
...
class Validator:
    ...
    def validate_translation_key(
        ...
        if not is_non_empty_string(value):
            self.add_context_error(
                manifest_path, context, f"{field} must be a non-empty string"
            )
            return
    ...
    def validate_string_list(
        ...
        *,
        ...
        if not isinstance(value, list):
            self.add_context_error(
                manifest_path, context, f"{field} must be a list of strings"
            )
            return
        ...
        for index, item in enumerate(value):
            ...
            if item in seen:
                self.add_context_error(
                    manifest_path, context, f"{field} contains duplicate '{item}'"
                )
            seen.add(item)
    ...
    def validate_root_fields(
        ...
        for field in ROOT_STRING_FIELDS:
            if field not in manifest:
                ...
                self.add_error(
                    manifest_path, f"root field '{field}' must be a non-empty string"
                )
        ...
        if "tags" in manifest:
            self.validate_string_list(
                manifest_path, "root", "tags", manifest["tags"], allow_empty=False
            )
        ...
        expected_id = f"{manifest['author']}/{plugin_dir}"
        ...
        for field in ("version", "min_noctalia"):
            ...
            if is_non_empty_string(value) and not SEMVER_RE.fullmatch(value):
                self.add_error(
                    manifest_path, f"root field '{field}' must use MAJOR.MINOR.PATCH"
                )

    def validate_entry_path(
        self, manifest_path: Path, context: str, plugin_dir: Path, value: Any
    ) -> None:
        if not is_non_empty_string(value):
            self.add_context_error(
                manifest_path, context, "entry must be a non-empty string"
            )
            return
        ...
        if raw_path.is_absolute() or ".." in raw_path.parts:
            self.add_context_error(
                manifest_path, context, "entry must stay inside the plugin directory"
            )
            return
    ...
    def validate_launcher_fields(
        self, manifest_path: Path, context: str, entry: dict[str, Any]
    ) -> None:
        ...
        if "include_in_global_search" in entry and not isinstance(
            entry["include_in_global_search"], bool
        ):
            self.add_context_error(
                manifest_path,
                context,
        ...
        if "category" in entry:
            self.validate_launcher_categories(
                manifest_path, f"{context}.category", entry["category"]
            )

    def validate_launcher_categories(
        self, manifest_path: Path, context: str, categories: Any
    ) -> None:
        ...
        for index, category in enumerate(categories):
            ...
            if not isinstance(category, dict):
                self.add_context_error(
                    manifest_path, category_context, "must be a table"
                )
                continue
            ...
            for field in unknown:
                self.add_context_error(
                    manifest_path, category_context, f"unknown field '{field}'"
                )

            label = category.get("label")
            if not is_non_empty_string(label):
                self.add_context_error(
                    manifest_path, category_context, "label must be a non-empty string"
                )
            elif label in seen_labels:
                self.add_context_error(
                    manifest_path,
                    category_context,
                    f"duplicate category label '{label}'",
                )
            else:
            ...
            if not is_non_empty_string(category.get("glyph")):
                self.add_context_error(
                    manifest_path, category_context, "glyph must be a non-empty string"
                )
    ...
    def validate_panel_fields(
            ...
            if isinstance(value, str):
                if value != "fill":
                    self.add_context_error(
                        manifest_path,
                        context,
                        f'{field} must be a positive number or "fill"',
                    )
                else:
                    uses_fill = True
        ...
        if uses_fill and entry.get("placement", "floating") != "floating":
            self.add_context_error(
                manifest_path,
                context,
                'width/height "fill" requires placement = "floating"',
            )
        ...
        if "placement" in entry:
            ...
            if not is_non_empty_string(placement):
                self.add_context_error(
                    manifest_path, context, "placement must be a non-empty string"
                )
            elif placement not in PANEL_PLACEMENTS:
                valid = ", ".join(sorted(PANEL_PLACEMENTS))
                self.add_context_error(
                    manifest_path, context, f"placement must be one of: {valid}"
                )
        ...
        if "position" in entry:
            ...
            if not is_non_empty_string(position):
                self.add_context_error(
                    manifest_path, context, "position must be a non-empty string"
                )
            elif position not in PANEL_POSITIONS:
                valid = ", ".join(sorted(PANEL_POSITIONS))
                self.add_context_error(
                    manifest_path, context, f"position must be one of: {valid}"
                )

        if "open_near_click" in entry and not isinstance(
            entry["open_near_click"], bool
        ):
            self.add_context_error(
                manifest_path, context, "open_near_click must be a bool"
            )
    ...
    def validate_entries(
        ...
        for entry_type in ENTRY_TYPES:
            ...
            if not isinstance(entries, list):
                self.add_error(
                    manifest_path, f"'{entry_type}' must be an array of tables"
                )
                continue
            ...
            for index, entry in enumerate(entries):
                ...
                for field in unknown:
                    self.add_context_error(
                        manifest_path, context, f"unknown field '{field}'"
                    )

                entry_id = entry.get("id")
                if not is_non_empty_string(entry_id):
                    self.add_context_error(
                        manifest_path, context, "id must be a non-empty string"
                    )
                ...
                self.validate_entry_path(
                    manifest_path, context, plugin_dir, entry.get("entry")
                )
    ...
    def validate_default(
        ...
        if setting_type in {"string", "folder"} and not isinstance(default, str):
            ...
            self.add_context_error(
                manifest_path, context, "default must be a non-empty string"
            )
        elif setting_type == "string_list":
            self.validate_string_list(
                manifest_path, context, "default", default, allow_empty=True
            )
    ...
    def validate_options(
        ...
        if options is None:
            if setting_type == "select":
                self.add_context_error(
                    manifest_path, context, "select settings require options"
                )
            return []

        if setting_type != "select":
            self.add_context_error(
                manifest_path, context, "options is only valid for select settings"
            )

        if not isinstance(options, list):
            self.add_context_error(
                manifest_path, context, "options must be a list of tables"
            )
            return []
        ...
        for index, option in enumerate(options):
            ...
            for field in unknown:
                self.add_context_error(
                    manifest_path, option_context, f"unknown field '{field}'"
                )

            value = option.get("value")
            if not is_non_empty_string(value):
                self.add_context_error(
                    manifest_path, option_context, "value must be a non-empty string"
                )
            else:
                values.append(value)
                if value in seen:
    ...
    def validate_int_bounds(
        self, manifest_path: Path, context: str, setting: dict[str, Any]
    ) -> None:
        setting_type = setting.get("type")
        ...
        for field in ("min", "max", "step"):
            ...
            if setting_type != "int":
                self.add_context_error(
                    manifest_path, context, f"{field} is only valid for int settings"
                )
            elif not is_int(value):
                self.add_context_error(
                    manifest_path, context, f"{field} must be an integer"
                )
            else:
        ...
        if (
            "min" in bound_values
            and "max" in bound_values
            and bound_values["min"] > bound_values["max"]
        ):
            self.add_context_error(
                manifest_path, context, "min must be less than or equal to max"
            )

        if "step" in bound_values and bound_values["step"] <= 0:
            self.add_context_error(
                manifest_path, context, "step must be greater than zero"
            )
        ...
        if setting_type == "int" and is_int(default):
            ...
            if "max" in bound_values and default > bound_values["max"]:
                self.add_context_error(
                    manifest_path, context, "default must be less than or equal to max"
                )

    def validate_visible_when(
        self, manifest_path: Path, context: str, value: Any
    ) -> None:
        if value is None:
            return
        ...
        if not isinstance(value, dict):
            self.add_context_error(
                manifest_path, context, "visible_when must be a table"
            )
            return
        ...
        for field in unknown:
            self.add_context_error(
                manifest_path, context, f"visible_when has unknown field '{field}'"
            )

        if not is_non_empty_string(value.get("key")):
            self.add_context_error(
                manifest_path, context, "visible_when.key must be a non-empty string"
            )
    ...
    def validate_settings(
        ...
        if not isinstance(settings, list):
            self.add_context_error(
                manifest_path, context_prefix, "must be an array of tables"
            )
            return
        ...
        for index, setting in enumerate(settings):
            ...
            if not is_non_empty_string(key):
                self.add_context_error(
                    manifest_path, context, "key must be a non-empty string"
                )
            elif key in seen_keys:
                self.add_context_error(
                    manifest_path, context, f"duplicate setting key '{key}'"
                )
            else:
                seen_keys.add(key)
            ...
            if not is_non_empty_string(setting_type):
                self.add_context_error(
                    manifest_path, context, "type must be a non-empty string"
                )
                setting_type = ""
            elif setting_type not in SETTING_TYPES:
                self.add_context_error(
                    manifest_path, context, f"unsupported setting type '{setting_type}'"
                )
            ...
            option_values = self.validate_options(
                manifest_path, translations, context, setting
            )
            if setting_type in SETTING_TYPES:
                self.validate_default(
                    manifest_path, context, setting_type, setting, option_values
                )

            self.validate_int_bounds(manifest_path, context, setting)
            self.validate_visible_when(
                manifest_path, context, setting.get("visible_when")
            )

            if "advanced" in setting and not isinstance(setting["advanced"], bool):
                self.add_context_error(
                    manifest_path, context, "advanced must be a bool"
                )
    ...
    def validate_manifest(self, manifest_path: Path) -> None:
        ...
        if "setting" in manifest:
            self.validate_settings(
                manifest_path, translations, manifest["setting"], "setting"
            )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate official Noctalia plugin manifests."
    )
    parser.add_argument(
        "--root",
        type=Path,
